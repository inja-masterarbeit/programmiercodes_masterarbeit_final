...
%disp(price_cv);
var_cv_fine=log(var(payoffs_arith-b_l*(payoffs_MC-price_exact),1));
var_total_mlmc_cv=var(payoffs_arith-b_l*(payoffs_MC-price_exact),1)/N;
var_mc=var((payoffs_arith),1)/N;
 else
...
fprintf('b_l: %.10f\n', b_l);
var_mlmc_cv(l)=log(var((payoffs_arith-payoffs_c_arith)-b_l*(payoffs_MC- payoffs_c_MC-(price_exact-price_exact_grob)),1)); 
var_cv_fine(l+1)=log(var((payoffs_arith)-b_l*(payoffs_MC- -(price_exact)),1));
 payoffs_var_ijk(l+1)=log(var(payoffs_arith-payoffs_c_arith,1));
 var_total_mlmc_cv=var_total_mlmc_cv+var((payoffs_arith-payoffs_c_arith)-b_l*(payoffs_MC- payoffs_c_MC-(price_exact-price_exact_grob)),1)/N;
 var_mc=var_mc+var((payoffs_arith-payoffs_c_arith),1)/N;
 end
end
fprintf('Var CV: %.10f\n', var_total_mlmc_cv);
...
payoffs_fine_ijk = max(S_arith_avg - K, 0);
 ...
 if l==0
     ...
     % payoffs_var_ijk(l+1)=log(var(payoffs_fine_m,1));
 else 
h_coarse=T/(M0*M^(l-1));
