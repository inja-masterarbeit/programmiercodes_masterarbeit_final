function price1 = mlmc_mc_var_arith_heston_cv_milstein (r, sigma, K, S0, T,h,  M, N, dW)
...
for l=0:L
    N=N_l(l+1);
     ...
     S_f_ijk = zeros(N,M0* M^l+1);         % Speicher für Pfad
     v_f_ijk = zeros(N,M0*M^l+1);
     v_f_ijk(:,1)=v0;
     S_f_ijk(:,1) = S0;     % Startwert für jeden Pfad
     S_f_bs=zeros(N,M0*M^l+1);
     S_f_bs(:,1)=S0;
     h=T/(M0*M^l);
    
     n=M0*M^l; %dynamische Anzahl Beobachtungspunkte
    ...
    price_exact = (exp(m + 0.5*s2) * normcdf(d1) - K * normcdf(d2)); %undiskontierte Preis
    ...
    %arithmetischen asian options im Heston
    S_arith_avg = mean(S_f_ijk(:, 2:end), 2);
    payoffs_arith= max(S_arith_avg - K, 0);
  
    b_l = cov(payoffs_arith, payoffs_MC);
    b_l = b_l(1,2) / var(payoffs_MC);
    disp(var(payoffs_arith));
    %disp(b_l);
   if l==0
        price_cv=exp(-r*T)*mean(payoffs_arith-b_l*(payoffs_MC-price_exact));
        %disp(price_cv);
        var_cv_fine=log(var(payoffs_arith-b_l*(payoffs_MC-price_exact)));
        var_total_mlmc_cv=var(payoffs_arith-b_l*(payoffs_MC-price_exact))/N;
        var_mc=var((payoffs_arith))/N;
  else
      ...
      n=M0*M^(l-1);
       ...
       % Beobachtungspunkte gleichmäßig über [T/n, ..., T]
        ...
        S_arith_avg = mean(S_c_ijk(:, 2:end), 2);
        ...
        var_mlmc_cv(l)=log(var((payoffs_arith-payoffs_c_arith)-b_l*(payoffs_MC- payoffs_c_MC-(price_exact-price_exact_grob)))); 
        var_cv_fine(l+1)=log(var((payoffs_arith)-b_l*(payoffs_MC- -(price_exact))));
        payoffs_var_ijk(l+1)=log(var(payoffs_arith-payoffs_c_arith));
        var_total_mlmc_cv=var_total_mlmc_cv+var((payoffs_arith-payoffs_c_arith)-b_l*(payoffs_MC- payoffs_c_MC-(price_exact-price_exact_grob)))/N;
        var_mc=var_mc+var((payoffs_arith-payoffs_c_arith))/N;
   end
end
price1=price_cv;
...
for l=0:L
    ...
    payoffs_fine_m = max(S_arith_avg - K, 0);
     ...
     if l==0
         ...
         payoffs_var_m(l+1)=log(var(payoffs_fine_m));
        ...
        payoffs_var_m(l+1)=log(var(payoffs_fine_m-payoffs_coarse_m));
...
hold on;
plot(x(2:end), var_mlmc_cv, '-o',  'Color', 'black');
hold on;
plot(x, y1, '-x');
hold on;

plot(x, var_cv_fine, '-x')
xlabel('Level l');
ylabel('log_M Varianz');
legend( 'P^l-P^{l-1}(Euler)',  'P^l-P^{l-1}(Milstein)', 'P^l-P^{l-1}(IJK)', 'P^l-P^{l-1} (IJK, CV)', 'P^l (Euler)', 'P^l (IJK, CV)');
title('Levelweise Varianzabschätzung mit CV (IJK)');
