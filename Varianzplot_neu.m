...
N_levels = optimale_Nl(V_l, M, epsilon, T, M0);
...
for l=0:L
 ...
 if l==0
      ...
      var_ges=var(payoffs_fine)/N;
      var_ges_m=var(payoffs_fine_m)/N;
      var_ges_i=var(payoffs_fine_ijk)/N;
     ...
     payoffs_var_ijk(l+1)=log(var(payoffs_fine_ijk-payoffs_coarse_ijk,1))/log(M);
      ...
      var_ges=var_ges+var(payoffs_fine-payoffs_coarse)/N;
      var_ges_m=var_ges_m+var(payoffs_fine_m-payoffs_coarse_m)/N;
      var_ges_i=var_ges_i+var(payoffs_fine_ijk-payoffs_coarse_ijk)/N;
 end
...
figure;
hold on;
colors = lines(3);  % 3 Verfahren × 2 Varianten
...
plot(x(2:end),y, '--o');
hold off;
xlabel('Level l','FontWeight','bold','FontSize',10);
ylabel('log_M Varianz','FontWeight','bold','FontSize',10);
legend( 'P_l-P_{l-1}(Euler)', 'P_l-P_{l-1}(Milstein)', 'P_l-P_{l-1}(IJK)', ' P_l (Euler)', ' P_l (Milstein)', ' P_l (IJK)', 'Referenzlinie mit Steigung 1');
