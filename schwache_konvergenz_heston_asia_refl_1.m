function schwache_konvergenz_heston_asia_refl
    % Parameter
...
errors_r = zeros(length(M_list), P);
errors_m_r = zeros(length(M_list), P);
for p=1:P
        ...
        for i = 1:length(M_list)
            ...
            v_f_ijk(:,1)=v0;
             ...
             v_f_m_r= zeros(N, M+1);    
             v_f_m_r(:,1) = v0;
             S_fine_r= zeros(N, M+1);     
             S_fine_r(:,1) = S0;
             v_fine_r= zeros(N, M+1);    
             v_fine_r(:,1) = v0;
            ...
            for t = 1:M
                    ...
                    %Milstein reflection
                    v_f_m_r(:,t+1)=v_f_m_r(:,t)+kappa*(theta-v_f_m_r(:,t))*h+sigma*sqrt(v_f_m_r(:,t)).*dW2(:,t)+1/4*sigma^2*(dW2(:,t).^2-h);
                    neg_idx = v_f_m_r(:,t+1) < 0;
                    v_f_m_r(neg_idx,t+1) = -v_f_m_r(neg_idx,t+1);  % reflection
                    S_f_m_r(:,t+1) = S_f_m_r(:,t) + r * S_f_m_r(:,t) * h + sqrt(v_f_m_r(:,t)).* S_f_m_r(:,t) .* dW1(:,t)+1/2*S_f_m_r(:,t).*v_f_m_r(:,t).*(dW1(:,t).^2-h);

                    %Euler reflection
                     v_fine_r(:,t+1)=abs(v_fine_r(:,t)+kappa*(theta-v_fine_r(:,t))*h+sigma*sqrt(v_fine_r(:,t)).*dW2(:,t));
                    
                    %Berechne erst v(t+dt) und daraus dann S
                    S_fine_r(:,t+1) = S_fine_r(:,t) + r * S_fine_r(:,t) * h + sqrt(v_fine_r(:,t)).* S_fine_r(:,t) .* dW1(:,t);
            ...
            payoffs_fine = max(S_arith_avg - K, 0);
             ...
             payoffs_f_m_r = max(S_arith_avg - K, 0);
            ...
            v_c_ijk(:,1)=v0;
             ...
             S_c_m_r = zeros(N,M/2+1);
             S_c_m_r(:,1)=S0;
             v_c_m_r = zeros(N,M/2+1);
             v_c_m_r(:,1)=v0;
             S_coarse_r = zeros(N,M/2+1);
             S_coarse_r(:,1)=S0;
             v_coarse_r = zeros(N,M/2+1);
             v_coarse_r(:,1)=v0;
             for t = 1:(M/2)
                    ...
                    %Milstein reflection
                    v_c_m_r(:,t+1)=v_c_m_r(:,t)+kappa*(theta-v_c_m_r(:,t))*h_coarse+sigma*sqrt(v_c_m_r(:,t)).*dW2_coarse(:,t)+1/4*sigma^2*(dW2_coarse(:,t).^2-h_coarse);
                    neg_idx = v_c_m_r(:,t+1) < 0;
                    v_c_m_r(neg_idx,t+1) = -v_c_m_r(neg_idx,t+1);  % reflection
                    S_c_m_r(:,t+1) = S_c_m_r(:,t) + r * S_c_m_r(:,t) * h_coarse + sqrt(v_c_m_r(:,t)).* S_c_m_r(:,t) .* dW1_coarse(:,t)+1/2*S_c_m_r(:,t).*v_c_m_r(:,t).*(dW1_coarse(:,t).^2-h_coarse);

                    %Euler reflection
                    v_coarse_r(:,t+1)=v_coarse_r(:,t)+kappa*(theta-v_coarse_r(:,t))*h_coarse+sigma*sqrt(v_coarse_r(:,t)).*dW2_coarse(:,t);
                    neg_idx = v_coarse_r(:,t+1) < 0;
                    v_coarse_r(neg_idx,t+1) = -v_coarse_r(neg_idx,t+1);  % reflection
                    S_coarse_r(:,t+1) = S_coarse_r(:,t) + r * S_coarse_r(:,t) * h_coarse + sqrt(v_coarse_r(:,t)).* S_coarse_r(:,t) .* dW1_coarse(:,t);
            ...
            payoffs_coarse = max(S_arith_avg - K, 0);
             ...
             payoffs_c_m_r = max(S_arith_avg - K, 0);
            ...
            errors_r(i, p) = abs(mean(payoffs_fine_r)-mean(payoffs_coarse_r));
            errors_m_r(i,p) = abs(mean(payoffs_f_m_r )-mean( payoffs_c_m_r));
    ...
    errors_r_ges=sum(errors_r,2)*1/P;
    errors_m_r_ges=sum(errors_m_r,2)*1/P;
    % Ausgabe
    ...
    loglog(M_list, errors_m_ges, '^-', 'LineWidth', 2);
    hold on;
    
    loglog(M_list, errors_ijk_ges, 'd-', 'LineWidth', 2);
    hold on;
    loglog(M_list, errors_r_ges, '*-', 'LineWidth', 2);
    hold on;
    loglog(M_list, errors_m_r_ges, '+-', 'LineWidth', 2);
    hold on;
    xlabel('Anzahl Zeitschritte M'); ylabel('$\mid E\left[ \left( f(\hat{S}_{h}) -f( \hat{S}_{2h}) \right)\right] \mid$', 'Interpreter', 'latex');

    title('Schwache Konvergenz (Reflection)');
    legend('Euler', 'Milstein', 'IJK', 'Euler, Reflection', 'Milstein, Reflection');
    grid on;
